...
class Nude():
    ...
    def __init__(self, path_or_image):
        # 若path_or_image为Image.Image类型实例，直接复制
        if isinstance(path_or_image, Image.Image):
            self.image = path_or_image
        # 若path_or_image为str类型，则视为图片路径，打开图片
        ...
        # 判断是否为单通道图片（灰度图），是则将灰度图转换为RGB图
        if len(bands) == 1:
            # 新建相同大小的RGB图像
            new_img = Image.new("RGB", self.image.size)
            # 拷贝灰度图self.image到RGB图 new_image.paste（PIL自动进行通道映射）
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # 给每个像素分配唯一id值（1,2,3...height*width)
                # 注意x,y的值从零开始
                ...
                # 存在相邻像素索引的列表，存放顺序为由大到小，顺序改变有影响
                # 注意 _id 是从 1 开始的，对应的索引则为 _id-1
                check_indexes = [
                    _id - 2,  # 当前像素左方的像素
                    _id - self.width - 2,  # 当前像素左上方的像素
                    _id - self.width - 1,  # 当期像素的上方的像素
                ...
                for index in check_indexes:
                    ...
                    if self.skin_map[index].skin:
                        # 若相邻像素与当前像素的 region 均为有效值，且二者不同，则尚未添加相同的合并任务
                        if (self.skin_map[index].region is not None and region is not None and region != -1 and
                ...
                if region == -1:
                    # 更改属性为新的区域号，注意元祖是不可变的类型，不能直接更改属性

...
# device
dev_name = 'MIX 2'
registrationId = 'a7654fcb94e91d52d174ffb26349876'
imei = '9900102565449'
...
class YqUser:
    ...
    def show_statis(self):
        ...
        print('Exp: {}, Points: {}\r\n'.format(resp_data['resultMap']['point'],
                                               resp_data['resultMap']['treasure']))
    ...
    def browse_news(self):
        ...
        for news in news_list:
            ...
            try:
                ...
                resp_data = get_resp_data('/app/newsActivity/service.do', data_server, req_headers,
                                          req_data_val)
            except Exception as e:
    ...
    def post_comments(self):
        ...
        for news in news_list:
            ...
            try:
                req_data_val = {
                    'content': comments[random.randint(0,
                                                       len(comments) - 1)],
                    'msgId': 'APP039',
                    'targetId': news['id'],
                    'targetType': '2',
    ...
    def watch_one_video(self, video, quick):
        ...
        print('Watching video:  time: {}(s)'.format(video_time), flush=True)

        now = datetime.today()
        begin_time = '{}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}'.format(now.year, now.month, now.day,
                                                                         now.hour, now.minute, now.second)

        wait_time(video_time)

        now = datetime.today()
        end_time = '{}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}'.format(now.year, now.month, now.day, now.hour,
        ...
        try:
            ...
            video_data = {
                'userId': self.id,
                'newsId': str(video['id']),
                'playBeginTime': begin_time,
                'playEndTime': end_time,
                'playTime': str(video_time),
                'totalDuration': 0,
                'totalTime': video_time,
                'networkType': 1
            }
...
def main():
    ...
    while True:
        ...
        print('4. 分享新闻 -- 将最新的5条新闻的标题分享到"华哥专属"圈子中，需要手动加入圈子.')
